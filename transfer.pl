...
sub processTask
{
   ...
   eval
   {
      ...
      if($task->{orig_link})
      {
         ...
         die("No orig dir") if !$orig_dir;
         mkdir($orig_dir, 0755) || die("Couldn't create directory at $orig_dir: $!") if ! -d $orig_dir;
...
sub lwp_download
{
   ...
   mkdir($workdir, 0777) || die("Can't create directory at $workdir: $!") if ! -d $workdir;
